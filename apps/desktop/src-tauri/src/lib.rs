...
fn skills_root(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    ...
    fs::create_dir_all(&root).map_err(|e| {
        format!("Could not create skills metadata directory: {e}")
    })?;
...
fn is_valid_skill_id(id: &str) -> bool {
    ...
    let Some((source, slug)) =
        id.split_once('@')
    else {
        return false;
    };

    let source_parts: Vec<&str> =
...
fn parse_skill_result_line(
    ...
    let before_installs = line
        .split_once(" installs")
        .map(|(left, _)| left)?
        .trim();

    let mut parts =
        before_installs.split_whitespace();
    ...
    let (_, source, slug) =
        skill_id_parts(id).ok()?;

    let installs =
        parse_installs_value(installs_text);
...
fn parse_find_output(
    ...
    while index < lines.len() {
        ...
        let Some(mut skill) =
            parse_skill_result_line(current_line)
        else {
            index += 1;
            continue;
        };
...
fn canonical_global_skills_directory(
...
fn find_skill_directory_by_slug(
    slug: &str,
) -> Result<PathBuf, String> {
    let canonical =
        canonical_global_skills_directory()?;
...
fn read_global_skill(
    ...
    let directory =
        find_skill_directory_by_slug(&slug)?;
...
async fn search_skills(
    ...
    if !result.success {
        return Err(command_error(
            &result,
            "Could not search Skills registry.",
        ));
    }
...
async fn install_skill(
    ...
    */
    ...
    .map_err(|e| {
        format!(
            "Could not save Nexora metadata: {e}"
        )
    })?;
...
async fn uninstall_skill(
    ...
    */
    ...
    if metadata_file.exists() {
        ...
        .map_err(|e| {
            format!(
                "Could not remove Nexora metadata: {e}"
            )
        })?;
    }

    println!(
        "Nexora metadata removed."
    );
...
async fn update_skill(
    ...
    */
    ...
    .map_err(|e| {
        format!(
            "Could not update Nexora metadata: {e}"
        )
    })?;
...
fn parse_installed_json(
    ...
    */
    ...
    {
        return parse_installed_json_value(
            value,
        );
    }
    ...
    let items =
            ...
            value.get("skills")
        {
            skills
                ...
                .cloned()
                .ok_or_else(|| {
                    "Invalid Skills CLI JSON: 'skills' is not an array."
                        .to_string()
                })?
        } else {
            return Err(
...
async fn read_installed_skill(
    ...
    let content =
        read_global_skill(&id)?;

    println!(
        "Skill read successfully."
    );
...
async fn audit_skill(
    ...
    let url =
        format!(
            "https://skills.sh/api/v1/skills/audit/{source}/{slug}"
        );
